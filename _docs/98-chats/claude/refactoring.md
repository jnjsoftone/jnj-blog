...
- 아래의 파일들에서 자동화 블로그 플랫폼 내용이 치환되지 않았네요.  자동화 블로그 플랫폼 값은 -d "일맥 업무/프로젝트 관리 웹앱" 에서 -d 옵션값("일맥 업무/프로젝트 관리 웹앱")를 사용하면 됩니다.
...
```
  "description": "자동화 블로그 플랫폼 nextjs server",
```

> `backend/nodejs/package.json`

```
  "description": "자동화 블로그 플랫폼 backend service",
```
